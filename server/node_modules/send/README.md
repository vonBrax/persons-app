...
Enable or diable the `immutable` directive in the `Cache-Control` response header, defaults to `false`. If set to
...
The `pipe` method is used to pipe the response into the Node.js HTTP response object, typically
